...
/* ═══════════════════════════════════════════════════════════════════════════ */
export default function ReceivablePrint() {
  ...
  const { data, isLoading } = useQuery<ReceivablesResponse>({
    queryKey: ['receivable-print', from, to],
    queryFn: () =>
  ...
  const totalInvoiced: number = rows.reduce(
    (sum, r) => sum + Number(r.grandTotal || 0),
    0
  );
  const totalCollected: number = rows.reduce(
    (sum, r) => sum + Number(r.paymentReceived || 0),
    0
  );
  ...
  const handleShare = async (): Promise<void> => {
    if (navigator.share) {
      try {
        await navigator.share({ title: 'Receivables Report', url: window.location.href });
      } catch (_) {
                ...
                {rows.length === 0 ? (
                  <div className="border border-dashed border-slate-300 rounded-md py-16 flex flex-col items-center gap-2 text-center">
                    ...
                    <p className="text-slate-400 text-xs max-w-sm">
                      No receivable records found for selected period.
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    <p className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">
                      Receivables Detail — {rows.length} Invoice{rows.length !== 1 ? 's' : ''}
                    </p>
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="border border-slate-200 p-1.5 text-center text-slate-500">
                                {r.invoiceDate
                                  ? format(new Date(r.invoiceDate), 'dd/MM/yyyy')
                                  : '—'}
                              </td>
