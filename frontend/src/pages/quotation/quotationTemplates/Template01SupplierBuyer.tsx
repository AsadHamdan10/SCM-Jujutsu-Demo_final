// ── Template01SupplierBuyer.tsx ───────────────────────────────
// Quotation Format 1: Supplier + Buyer.
// Changes from spec:
//   1. Supplier (From) box REMOVED — supplier info already in header
//   2. Buyer (To) box is now full-width
//   3. Signature: Company Name above "Authorised Signatory", right-aligned
//   4. Meta column (Quotation No / Date / Valid Till) uses a proper
//      table with fixed label + value columns — no more clipping
...
export default function Template01SupplierBuyer({ data, settings: settingsProp }: Props) {
  const settings = settingsProp ?? getInvoiceSettings();
  const { supplier, buyer, items, totals } = data;
  ...
  return (
    <div
      ...
      style={{
        ...
        color: '#111',
        background: '#fff',
        padding: '28px 32px 60px',   // bottom padding leaves room for footer bar
        width: '794px',
        minHeight: '1123px',
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px', gap: '24px' }}>
        ...
        {/* Right: table layout so label column and value column each have fixed width → no clipping */}
      ...
      {/* ── BUYER (TO) — full width. Supplier box intentionally removed
             because the supplier info is already shown in the header above. ── */}
      <div style={{ border: '1px solid #bfdbfe', borderRadius: '4px', padding: '10px 14px', marginBottom: '14px' }}>
        <div style={{ fontSize: '10px', fontWeight: '700', color: ACCENT_LIGHT, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '5px' }}>
          Buyer (To)
        </div>
        <div style={{ fontWeight: '700', fontSize: '12px', color: '#111', marginBottom: '3px' }}>
          {buyer.buyerName}
        </div>
        {buyer.contactPerson && (
          <div style={{ color: '#374151', fontSize: '10.5px' }}>{buyer.contactPerson}</div>
        )}
        <AddressBlock
          text={buyerAddress}
          settings={settings}
          style={{ color: '#374151', fontSize: '10.5px', lineHeight: '1.6' }}
        />
        {buyer.mobile && <div style={{ color: '#374151', fontSize: '10.5px' }}>Mobile : {buyer.mobile}</div>}
        {buyer.email  && <div style={{ color: '#374151', fontSize: '10.5px' }}>Email : {buyer.email}</div>}
        {buyer.gstin  && <div style={{ color: '#374151', fontSize: '10.5px', fontFamily: 'monospace' }}>GSTIN : {buyer.gstin}</div>}
      </div>
      ...
      {/* ── Product table: # | Product | HSN | Qty | Unit | Rate (₹) | Amount (₹) ── */}
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '12px', fontSize: '10.5px' }}>
        ...
        <tbody>
          ...
          {/* Filler rows so the table never looks empty */}
